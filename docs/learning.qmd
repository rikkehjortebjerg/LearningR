...
# Task 1.
post_meal_data |>
  filter(BMI >= 25 & BMI <= 49 & Group == "FDR")
```
```{r}
# Task 2.
post_meal_data |>
...
tidied_post_meal_data |>
  filter(BMI < 30) |>
  ggplot(aes(x = Age, y = insulin_glucose_ratio, colour = Group)) +
  geom_point()
```
...
## Exercise 21.6: Changing colours
```{r}
# Tasks 1. and 2.
post_meal_data |>
...
## Calculate summary statistics, min and max
```{r}
tidied_post_meal_data |>
  summarise(
